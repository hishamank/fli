...
def _execute_date_search(params: DateSearchParams) -> dict[str, Any]:
    ...
    try:
        has_min = params.min_duration is not None
        has_max = params.max_duration is not None
        has_trip = params.trip_duration is not None

        if (has_min or has_max) and has_trip:
            return {
                "success": False,
        ...
        if not has_trip and not has_min and not has_max:
            params.trip_duration = 3

        if (has_min or has_max) and not params.is_round_trip:
            return {
                "success": False,
        ...
        if params.min_duration is not None or params.max_duration is not None:
            ...
            if actual_min > actual_max:
                return {
                    "success": False,
                    "error": (
                        f"min_duration ({actual_min}) must not exceed "
                        f"max_duration ({actual_max})"
                    ),
                    "dates": [],
                }
